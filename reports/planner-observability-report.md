# Planner Observability Report

Generated at: 2026-07-07T07:31:02.200Z

## Summary

- totalRuns: 0
- deepseekRuns: 0
- fallbackRuns: 0
- fallbackRate: 0
- repairAppliedRuns: 0
- repairRate: 0
- averageDurationMs: 0
- p95DurationMs: 0
- averagePromptTokens: 0
- averageCompletionTokens: 0
- averageTotalTokens: 0
- estimatedTotalCost: 0 CNY
- estimatedCostNote: pricing_not_configured; estimate_only; actual_costs_follow_provider_invoice

## Error Counts

- none: 0

## Failure Analysis

- provider_402: 0
- provider_429: 0
- provider_timeout: 0
- invalid_json: 0
- schema_validation_failed: 0
- repair_failed: 0

## Quality Signals

- averageFinalDays: 0
- daysMismatchCount: 0
- zeroMainActivityDayCount: 0
- averageMainActivitiesPerDay: 0
- averageTotalItemsPerDay: 0

## Eval Report

- mode: offline
- totalCases: 12
- passedCases: 12
- failedCases: 0
- averageScore: 100
- hardFailureCount: 0
- fallbackCount: 12
- repairAppliedCount: 0

## Recommendations

- No immediate observability action is required.

## Notes

- Cost is an estimate only. Actual costs must be checked in the provider invoice.
- Reports do not store API keys, tokens, sessions, full prompts, or full model responses.
- Vercel serverless local file writes are not long-term persistence.
